...
struct PartyMenuInternal
{
    ...
    u32 messageId:14;
    u8 windowId[3];
    u8 actions[11]; //was 8, +1 for nicknames, +2 for move relearn
    u8 numActions;
...
static void CursorCb_Moves(u8); //added for move relearner
...
void Task_HandleChooseMonInput(u8 taskId)
{
    if (!gPaletteFade.active && MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        ...
        case B_BUTTON: // Selected Cancel / pressed B
            ...
            break;
        case SELECT_BUTTON: // Lunos's Quick Swap
            DestroyTask(taskId);
            break;
        case START_BUTTON:
            if (sPartyMenuInternal->chooseHalf)
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (!InBattlePike())
    {
        ...
        if (GetNumberOfRelearnableMoves(&mons[slotId]) != 0) {//added for move relearner
...
//added for move relearner
static void CursorCb_Moves(u8 taskId)
{
    PlaySE(SE_SELECT);
	FlagSet(FLAG_MOVE_RELEARN_PARTY);
...
{
    gPaletteFade.bufferTransferDisabled = TRUE;
    //gPartyMenu.slotId = gLastViewedMonIndex; //removed for party menu nickname
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (!(sInitialLevel >= GetCurrentLevelCap() && (B_RARE_CANDY_CAP || (*itemPtr != ITEM_RARE_CANDY)))) //Only Rare Candy item can bypass cap
    {
    ...
    else
    {
        ...
        if (sFinalLevel > sInitialLevel)
        {
            ...
            if (holdEffectParam == 0) // Rare Candy (or Candy Jar)
            {
...
//Mints, based on ghoulslash implementation
#define tState          data[0]
#define tSpecies        data[1]
...
static void Task_Mints(u8 taskId)
{
    ...
    case 4:
        ...
        break;
    case 5:
        SetMonData(&gPlayerParty[tMonId], MON_DATA_SHEEN, &tNewNature);
        CalculateMonStats(&gPlayerParty[tMonId]);
