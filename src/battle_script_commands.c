...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    { ITEM_TINY_MUSHROOM,   {  35,   _,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_PRETTY_WING,     {  25,  10,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_HEAL_POWDER,     {   8,  30,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_STARDUST,        {   8,  10,  30,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_STAR_PIECE,      {   8,  10,   9,  30,   _,   _,   _,   _,   _,   _, } },
    { ITEM_BIG_MUSHROOM,    {   3,  10,   9,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_REVIVAL_HERB,    {   3,  10,   9,   9,  30,   _,   _,   _,   _,   _, } },
    { ITEM_LUCKY_EGG,       {   3,   3,   9,   8,   9,  30,   _,   _,   _,   _, } },
    { ITEM_REVIVE,          {   3,   3,   3,   8,   8,   9,  30,   _,   _,   _, } },
    { ITEM_ENERGY_ROOT,     {   3,   3,   3,   4,   8,   9,   8,  30,   _,   _, } },
    ...
    { ITEM_RED_SHARD,       {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_BLUE_SHARD,      {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_GREEN_SHARD,     {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_YELLOW_SHARD,    {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        if (odds > 254) // mon caught
        {
            ...
            else if (ballId == ITEM_DREAM_BALL)
            {
                u8 hiddenAbility = 2;
...
void BS_TryHealPulse(void)
{
    ...
    {
        if (GetBattlerAbility(gBattlerAttacker) == ABILITY_MEGA_LAUNCHER && (IsPulseMove(gCurrentMove) || IsBallisticMove(gCurrentMove)))
