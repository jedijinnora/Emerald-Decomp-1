...
void Overworld_ResetBattleFlagsAndVars(void)
{
    ...
    //FlagClear(B_FLAG_NO_BAG_USE); // Jinnora: this flag should activate in all trainer battles, it is set with difficulty menu
    FlagClear(B_FLAG_NO_CATCHING);
    FlagClear(B_FLAG_NO_RUNNING);
    FlagClear(B_FLAG_DYNAMAX_BATTLE);
    FlagClear(B_FLAG_SKY_BATTLE);
    FlagClear(FLAG_USE_NEXT_LEVEL_CAP); //Jinnora: level cap flag used in boss battles
    FlagClear(FLAG_NO_LEVEL_REDUCTIONS);  //Level restriction flag used in gyms
...
static bool8 CanLearnFlashInParty(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (!GetMonData(&gPlayerParty[i], MON_DATA_SPECIES, NULL))
            break;
        if (!GetMonData(&gPlayerParty[i], MON_DATA_IS_EGG) && CanLearnTeachableMove(GetMonData(&gPlayerParty[i], MON_DATA_SPECIES, NULL), MOVE_FLASH))
            return TRUE;
    }
    return FALSE;
}
...
static UNUSED bool16 IsInfiltratedWeatherInstitute(struct WarpData *warp)
{
    return FALSE;
}

//Jinnora: removed this
static UNUSED bool16 IsInflitratedSpaceCenter(struct WarpData *warp)
{
    return FALSE;
}
...
void CB2_NewGame(void)
{
    ...
    NewGameInitData();
    InitTimeBasedEvents();  //Jinnora adding call to set clock time here
    ResetInitialPlayerAvatarState();
    PlayTimeCounter_Start();
    ScriptContext_Init();
    UnlockPlayerFieldControls();
    //Jinnora removed for new intro:
