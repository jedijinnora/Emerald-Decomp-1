...
#define B_LIGHT_BALL_ATTACK_BOOST   GEN_CUSTOM // In Gen4+, Light Ball doubles the power of physical moves in addition to special moves. In GenC+, also buffs Pichu by 1.5x and Raichu by 1.2x.
...
#define B_HIDDEN_POWER_DMG          GEN_CUSTOM // In Gen6+, Hidden Power's base power was set to always be 60. Before, it was determined by the mon's IVs.
...
#define B_DARK_VOID_FAIL            GEN_CUSTOM // In Gen7+, only Darkrai can use Dark Void. In GenC, only Smeargle cannot use Dark Void
...
#define B_FLAG_NO_RUNNING           FLAG_NO_ESCAPE          // If this flag is set, the ability to escape from wild battles is disabled. Also makes Roar/Whirlwind and Teleport (under Gen8) fail.
...
#define B_VAR_STARTING_STATUS       VAR_START_TERRAIN   // If this var has a value, assigning a STATUS_FIELD_xx_TERRAIN to it before battle causes the battle to start with that terrain active.
